...
namespace API.Hubs {
    public class ChatHub : Hub{
        ...
        public async Task CreateChannel(string serverId, string jsonChannel) {
            Channel channel = JsonConvert.DeserializeObject<Channel>(jsonChannel);
            Program.mainDatabase.Channel.Add(channel);
            Program.mainDatabase.SaveChanges();
            channel = Program.mainDatabase.Channel.Last();
            jsonChannel = JsonConvert.SerializeObject(channel);
            await Clients.Group(serverId).SendAsync(ClientMethod.GetNewChannel, jsonChannel);
        }
